...
if($row = $result->fetch_assoc()){
  ...
  if($resulta->num_rows > 0) {
    ...
    if($rowa = $resulta->fetch_assoc()){
      $sql2 = "SELECT COUNT(eleave.ID) AS cid FROM employees_demas LEFT JOIN eleave ON employees_demas.CNOEE = eleave.CNOEE AND eleave.MNOTES = 'recommended' WHERE employees_demas.DRESIGN = '0000-00-00' AND employees_demas.CDIVISION = '$divi'";
            ...
            <a href="recommend" class="card load text-dark new-bg">
              <div class="card-body text-center <?php echo ($cnt > 0) ? 'text-danger' : 'text-success'; ?>">
                ...
                <b style="font-size: 15px;">
                  ...
                  <?php echo ($chead == 0) ? 'Recommendation' : 'Approval'; ?> 
                </b>
              </div>
            </a>
          </div>
