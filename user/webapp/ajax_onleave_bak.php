...
foreach ($result as $value) {
  foreach ($result1 as $value1) {
    if ($value1['CNOEE'] == $value['CNOEE'] && date("Y-m", strtotime($value1['DLEAVE'])) == sprintf("%04d-%02d", $year, $month)) {
      if ($value1['DLEAVE'] == $value1['DLEAVE2']) {
        foreach ($result2 as $value2) {
          if ($value1['CCDLEAVE'] == $value2['ID']) {
            $dayKey = $value1['DLEAVE'];
            $data[$dayKey][] = ['cname' => $value['CNAME'],'cleave' => $value2['leave_type'],'type' => $value1['CCDLEAVE'],'nhours' => ($value1['NDAYS'] == '0.50' && $value1['NHOURS'] == '10') ? '(Morning)' : (($value1['NDAYS'] == '0.50' && $value1['NHOURS'] == '01') ? '(Afternoon)' : ''),'status' => $value1['MNOTES']];
          }
        }
      } else {
    ...
    } else if ($value1['CNOEE'] == $value['CNOEE'] && date("Y-m", strtotime($value1['DLEAVE'])) <= sprintf("%04d-%02d", $year, $month) && date("Y-m", strtotime($value1['DLEAVE2'])) >= sprintf("%04d-%02d", $year, $month)) {
      $start = new DateTime($value1['DLEAVE']);
      $end = new DateTime($value1['DLEAVE2']);
      $end->modify('+1 day');
      $interval = new DateInterval('P1D');
