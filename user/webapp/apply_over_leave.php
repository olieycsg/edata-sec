              ...
              <div class="nk-header-wrap">
                <div class="nk-header-logo ms-n1">
                  <div class="nk-sidebar-toggle">
                    <a href="over_leave" style="display: inline-block; margin-right: 10px; text-align: center;" aria-label="Back">
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        $.ajax({
          url: 'api_main',
          type: 'POST',
          data: {add_leave_two:start,final:final,cnoee:cnoee,nhours:nhours,remarks:remarks,cname:cname,staff:staff,email:email,mnoee:mnoee,fnoee:fnoee,cjob:cjob},
          beforeSend: function(){    
            Swal.fire({
              title: 'PROCESSING',
          ...
          success: function(response) {
            if(cjob == 'CEO'){
              Swal.fire('LEAVE APPROVED').then(() => {
                window.location.href = 'over_leave';
              });
            }else{
              window.location.href = 'over_leave_pending?cmid=<?php echo $emid; ?>';
            }
          }
        });
      }
