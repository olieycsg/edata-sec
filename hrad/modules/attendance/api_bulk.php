    ...
    if ($type === "Attend") {
      ...
      if ($val['cint'] != '00:00:00' && $val['cout'] != '00:00:00') {
        ...
        if ($status !== 1) {
          ...
          if (strtotime($val['cint']) <= strtotime("09:00") && strtotime($val['cout']) < strtotime("18:00") && $val['rang'] < 540) {
            $early = gmdate("H:i:s", (540 - $val['rang']) * 60);
            $type = 'Incomplete';
            $stat = 'No Clock-OUT';
          }
...
/*$formatted = 480 - $val['rang'];
$hours = floor($formatted / 60);
$minutes = $formatted % 60;
$early = ($hours ? $hours.'h ' : '').$minutes.'m';
$type = 'Incomplete';
$stat = 'No Clock-OUT';*/
